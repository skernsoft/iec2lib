...
IEC::IEC(byte deviceNumber) :
	...
	m_clockPin(DEFAULT_CLOCK_PIN), m_srqInPin(DEFAULT_SRQIN_PIN)
...
IEC::ATNCheck IEC::checkATN(ATNCmd& cmd)
{
	...
	if(not readATN()) {
		...
		if(c == (ATN_CODE_LISTEN bitor m_deviceNumber)) {
			...
			if((c bitand 0xF0) == ATN_CODE_DATA and (c bitand 0xF) not_eq 15) {
...
void IEC::setPins(byte atn, byte clock, byte data, byte srqIn)
{
	m_atnPin = atn;
	m_clockPin = clock;
	m_dataPin = data;
